...
import type { IItemVersion, IStoreVersions, ResultTypeAt } from "./types/interfaces";
...
class StoreVersions<T> implements IStoreVersions<T> {
	...
	#getCorrectIndex(indexVersion?: IndexForAtMethod): number {
		...
		const isAnyTree =
			this.typeStructure === RED_BLACK_TREE || this.typeStructure === SET_STRUCTURE;
	...
	#atForHashTable(indexVersion?: IndexForAtMethod): ResultTypeAt<T> {
		...
		this.selectedVersion =
			indexVersion === "+1" ? this.selectedVersion + 1 : this.selectedVersion - 1;
	...
	#atForPointerMachineModel(indexVersion?: IndexForAtMethod): ResultTypeAt<T> {
		...
		if (this.typeStructure === RED_BLACK_TREE || this.typeStructure === SET_STRUCTURE) {
			...
			const recursivelyClone = this.#recursivelyCloneAllNodesForTree(correctNode);
